WITH [ProductCategory_hierarchy]([Id], cat) AS 
	(SELECT [ProductCategory].[Id] AS [Id],
		 'ProductCategory ' + CAST([ProductCategory].[Id] AS VARCHAR(max)) + ':' + [ProductCategory].[Name] AS cat
	FROM [ProductCategory]
	WHERE [ProductCategory].[Parent] IS NULL
	UNION
	ALLSELECT [ProductCategory].[Id] AS [Id],
		 [ProductCategory_hierarchy].cat + '&&ProductCategory ' + CAST([ProductCategory].[Id] AS VARCHAR(max)) + ':' + [ProductCategory].[Name] AS cat
	FROM [ProductCategory]
	JOIN [ProductCategory_hierarchy]
		ON [ProductCategory_hierarchy].[Id] = [ProductCategory].[Parent]), [PrevPurchases] AS 
...
JOIN [Store] AS [Store_1]
	ON [Store].[Parent_1CID] = [Store_1].[1C_Id] LEFT OUTER
JOIN [PrevPurchases]
	ON [PrevPurchases].[AccountID] = [LoyaltyProgramSales].[AccountID]
		AND [PrevPurchases].[Period] = [LoyaltyProgramSales].[Period]
